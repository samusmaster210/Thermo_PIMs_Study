...
output_dir = "Excel_Sigmoid_Results"
...
# Define hyperparameter search space for Bayesian Optimization (Sigmoid Kernel)
space = [
    Real(1e-6, 1e2, "log-uniform", name="alpha"),  # Regularization parameter
    Real(1e-6, 1e2, "log-uniform", name="gamma")  # Kernel coefficient
]
...
def objective(params):
    ...
    for train_idx, test_idx in kf.split(features):
        ...
        model = KernelRidge(kernel="sigmoid", alpha=alpha, gamma=gamma)
...
best_params.to_csv(os.path.join(output_dir, "Excel_Sigmoid_best_hyperparameters.csv"), index=False)
...
for train_idx, test_idx in kf.split(features):
    ...
    model = KernelRidge(kernel="sigmoid", alpha=best_alpha, gamma=best_gamma)
    model.fit(X_train, y_train)
...
cv_results.to_csv(os.path.join(output_dir, "Excel_Sigmoid_cv_results.csv"), index=False)
...
for dataset, actuals, preds, avg_r2, filename in zip(
    ...
    ["Excel_Sigmoid_training_parity_plot.png", "Excel_Sigmoid_testing_parity_plot.png"]
):
    plt.figure()
